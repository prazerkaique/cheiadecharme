...
const SCRATCH_TYPES: { value: PrizeType; label: string }[] = [
  ...
  { value: "yearly_service", label: "Servico Anual" },
...
export function ScratchTab() {
  ...
  useEffect(() => {
    if (config?.scratch_prizes) setPrizes(config.scratch_prizes);
  }, [config?.scratch_prizes]);

  const handleSave = async () => {
    if (!store?.id) return;
    try {
      await save(store.id, { scratch_prizes: prizes });
      addToast("Premios da raspadinha salvos", "success");
    } catch {
  ...
  return (
    <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="mb-5 flex items-center gap-3">
        ...
        <h3 className="font-bold text-gray-900">Premios da Raspadinha</h3>
      </div>

      <PrizeEditor prizes={prizes} onChange={setPrizes} typeOptions={SCRATCH_TYPES} />
